...
- All your project code into a single main package (`potential-octo-adventure/`)
...
```bash
<root directory>
├── potential-octo-adventure/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `potential-octo-adventure`
...
2. Clone the `potential-octo-adventure` repository

3. Open a shell into your local `potential-octo-adventure` directory and run
    ```bash
    ./rename-template.sh potential-octo-adventure
    ```
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `potential-octo-adventure`, in this example)
...
13. Put your main (resp. test) code in `potential-octo-adventure/` (resp. `test/`)
...
This will execute the `__main__.py` file in the `potential-octo-adventure` package:
```bash
python -m potential-octo-adventure
```
